...
const SIZE = 1e8;
const EXP_MAX = 7;
const buf = selection.generate("test", SIZE);
const CLOCKS_PER_SEC = selection.getClocksPerSec();
const MAX_TIME = 1 * CLOCKS_PER_SEC;

const ALGORITHM_MAX = 3;
...
tasks.forEach((value) => {
    queue.push(() => {
        ...
        if (!ALGORITHM_ACTIVE[value.order]) {
            selection_result[value.order] = null;
            return [value, -1];
        }
        ...
        selection_result[value.order] = newBuf;
